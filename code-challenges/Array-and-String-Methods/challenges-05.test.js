...
const splitFoods = (recipe) => {
    ...
    recipe.ingredients.forEach(elm => {
        let splitString = elm.split(' ');
        let spliceString = splitString.splice(2);
        let joinString = spliceString.join(' ');
...
const stepActions = (recipe) => {
    let result = [];
    // Solution code here...
    recipe.steps.forEach(elemnt => {
        let theFirstIndex = elemnt.split(' ')[0];
        result.push(theFirstIndex);
    });
    return result;
};
...
const removeVowels = (str) => {
    // Solution code here...
    let vowels = ['i', 'I', 'a', 'A', 'o', 'O', 'e', 'E', 'u', 'U'];
...
const extractVowels = (str) => {
    ...
    for (let i = 0; i < stringArray.length; i++) {
        for (let j = 0; j < stringArray.length; j++) {
            if (stringArray[i].includes(vowels[j])) {
                arrVowels.push(vowels[j]);
            }
            if (stringArray[i] === vowels[j]) {
                stringArray.splice(i, 1);
            }
    ...
    let arrVowels2 = arrVowels.join('');
    let stringArray2 = stringArray.join('');
    extract = [stringArray2,arrVowels2];
    return extract;
};
